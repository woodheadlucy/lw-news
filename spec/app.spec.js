...
describe('/api', () => {
  ...
  describe('/topics/:topic/articles', () => {
    ...
    // it.only('POST status: 400 bad request if the author does not exist in the database', () => {
    //   const dodgyArticle = {
  ...
  describe('/articles/:article_id/comments/:comment_id', () => {
    ...
    // it.only('DELETE status: 404 when an attempt to delete a comment on a non existent article is made', () => request.delete('/api/aticles/lunch/comments/1').expect(404));
    // it.only('DELETE status: 404 when an attempt to delete comments on a non-existent comment id is made', () => request.delete('/api/articles/1/comments/90909090909').expect(404));

    // it.only('PATCH status: 404 if invalid comment id is used', () =>
    // request.patch('/api/articles/1/haha12').expect(404));
  });
