...
describe('/api', () => {
  ...
  describe('/api/topics/:topic/articles', () => {
    ...
    // SORT BY
    xit('GET status:200 and sorts a column by the order specified', () => request.get('/api/topics/mitch/articles?sort_by=title&order=desc').expect(200).then(({ body }) => {
      expect(body.articles[0].created_at).to.equal('2018 - 11 - 15 12: 21: 54.171 + 00');
    }));
  });
});
